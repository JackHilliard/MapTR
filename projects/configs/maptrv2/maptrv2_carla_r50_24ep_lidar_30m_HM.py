_base_ = [
    './maptrv2_carla_r50_24ep_lidar_30m.py',
]
#
...
#
# [30m] `point_cloud_range` is restated here because mmcv evaluates each
# config file in isolation -- the same reason `fixed_ptsnum_per_line` is
# repeated below. KEEP IT EQUAL TO THE BASE'S, which for this file means
# maptrv2_carla_r50_24ep_lidar_30m.py's +-15, NOT the 25m config's +-12.5.
# Nothing checks this: a stale +-12.5 here would parse, train, and silently
# price geometry error against the wrong tile while the model regressed
# against +-15.
#
# Editing this line alone does NOT resize the tile. bev_h_/bev_w_, the coder,
# the assigner's own pc_range, data.*.pc_range and the LiDAR sparse_shape are
# all bound in the base's namespace and do not follow a change made here.
# This value only tells the loss and the matching cost what geometry they are
# working in; everything below derives from it.
