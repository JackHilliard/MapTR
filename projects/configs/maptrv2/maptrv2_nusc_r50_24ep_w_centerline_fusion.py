...
#
# MapTRv2 camera + LiDAR fusion config (nuScenes, with centerline).
#
# NOTE: MapTRv2 has no camera-free / LiDAR-only path. The BEV is always built
# from camera features (LSSTransform); LiDAR features are only fused on top of
# the camera BEV via the ConvFuser (see transformer.get_bev_features). This
# config therefore enables BOTH modalities (modality='fusion'). The LiDAR
# encoder + fuser pieces are ported from the v1 config
# `maptr/maptr_tiny_fusion_24e.py`, which the MapTRv2 detector supports
# unchanged (extract_lidar_feat / voxelize are shared).
#
# No official MapTRv2 fusion checkpoint is released, so to *infer* you must
# supply a checkpoint trained with this (or an equivalent fusion) config.
plugin = True
...
# LiDAR branch geometry (kept separate from the map/coder voxel_size above);
# values taken from the v1 fusion config so the SparseEncoder sparse_shape lines
# up: x (30m/0.1=300), y (60m/0.1=600), z (8m/0.2=40 -> 41).
...
# LiDAR loading params (match the v1 fusion config).
reduce_beams=32
load_dim=5
use_dim=5
...
train_pipeline = [
    ...
    # Load the keyframe sweep first (used for depth GT), then accumulate sweeps
    # for the LiDAR branch, then range-filter the accumulated cloud.
    dict(
