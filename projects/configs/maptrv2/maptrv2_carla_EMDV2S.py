# AUTO-DERIVED from maptrv2_carla_ORIG.py. Single variable: the geometry
# LOSS becomes order-preserving (monotone) optimal transport, mode 'emdv2',
# at measured gradient parity with PtsL1Loss (58.9x -> 1.02x). Everything
# else -- matching cost (OrderedPtsL1Cost w=5), cls_cost (2.0), loss_dir
# (0.005), schedule, data -- is ORIG's, so a difference is the loss.
#
# The matching cost deliberately stays OrderedPtsL1Cost rather than the
# emdv2 cost: it is evaluated over every (query x gt x order) triple and the
# DTW there measured 14.2 s/iter vs the L1 baseline's 6.0 s -- 2.4x, which
# would not finish. It is also already ORDER-SENSITIVE, so it fixes the
# permutation-invariance defect in the assignment on its own; the 59x scale
# defect was only ever in the loss. Cost per matched pair in the loss is
# negligible (a handful of pairs vs ~35k cost entries).
...
ann_file_train = 'data/carla_30m/tile_center/carla_map_infos_train.pkl'
ann_file_val = 'data/carla_30m/tile_center/carla_map_infos_test.pkl'
ann_file_test = 'data/carla_30m/tile_center/carla_map_infos_test.pkl'
map_ann_file = 'data/carla_30m/tile_center/carla_map_gt.json'
...
data = dict(
    ...
    workers_per_gpu=6,
    train=dict(
        type='CustomCarlaLocalMapDataset',
        data_root='data/carla_30m/',
        ann_file='data/carla_30m/tile_center/carla_map_infos_train.pkl',
        pipeline=[
            dict(
                type='LoadCarlaPointsFromFile',
                coord_type='LIDAR',
    ...
    val=dict(
        type='CustomCarlaLocalMapDataset',
        data_root='data/carla_30m/',
        ann_file='data/carla_30m/tile_center/carla_map_infos_test.pkl',
        map_ann_file='data/carla_30m/tile_center/carla_map_gt.json',
        pipeline=[
            dict(
                type='LoadCarlaPointsFromFile',
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        loss_pts=dict(
            ...
            mode='emdv2',
            # 5.0 (PtsL1Loss) * sqrt(C=2): PtsL1Loss sums P*C elementwise
